                ...
                <div class="bg-zinc-200">
                    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8 bg-gray-300 rounded-md"
                        ...
                        <div class="flex items-center space-x-2 mb-4">
                            <label for="price-range" class="text-sm font-medium text-gray-700">Price Range:</label>
                            <input 
                                type="range" 
                                id="price-range-min" 
                                min="0" 
                            ...
                            />
                            <span class="text-sm text-gray-500">{{ formatPrice(filters.priceRange[0]) }} - {{ formatPrice(filters.priceRange[1]) }}</span>
                        </div>
                                                ...
                                                <section aria-labelledby="options-heading" class="mt-10">
                                                    ...
                                                    <form>
                                                        <div class="flex items-center space-x-1">
                                                            ...
                                                            <input type="text"
                                                                class="w-10 h-10 border border-gray-300 text-center rounded"
...
const categoryId = route.params.id; // Get category ID from route parameter
const categoryName = ref('');
const products = ref([]);
const apiUrl = inject("apiUrl");

console.log('url:', `products/category/${categoryId}`)
onMounted(async () => {
    try {
        // Fetch category name based on ID
        const categoryResponse = await axios.get(`${apiUrl}/categories/${categoryId}`);
...
const getProducts = () => {
    axios
        ...
        .then(function (response) {
            ...
            console.log('totalProducts:', totalProducts.value)
...
const filters = reactive({
  priceRange: [0, 100000000], // Initial price range (adjust as needed)
  // Add other filter criteria as needed (e.g., brand, category, etc.)
  // brand: '',
  // category: '',
});
...
const maxPrice = computed(() => {
  return Math.max(...products.value.map(product => product.price)) + 5000000;
});
...
const paginatedProducts = computed(() => {
  const startIndex = (currentPage.value - 1) * pageSize.value;
  const endIndex = startIndex + pageSize.value;
  return filteredProducts.value.slice(startIndex, endIndex); // Use filteredProducts
});
...
const filteredProducts = computed(() => {
  return products.value.filter(product => {
    ...
    return priceMatch; // && brandMatch && categoryMatch; // Combine all conditions
  });
});
